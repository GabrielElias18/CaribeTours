export const packages = [
  {
    id: 1,
    name: "Tour Cholón Full Day",
    image: "https://images.pexels.com/photos/2166559/pexels-photo-2166559.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Vive la experiencia más famosa del Caribe colombiano",
    price: "120.000",
    includes: [
      "Transporte marítimo ida y vuelta",
      "Almuerzo típico caribeño",
      "Bebidas ilimitadas",
      "Música y animación",
      "Snorkel y equipo",
      "Guía turístico"
    ]
  },
  {
    id: 2,
    name: "Islas del Rosario Premium",
    image: "https://images.pexels.com/photos/1268855/pexels-photo-1268855.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Paraíso natural a 45 minutos de Cartagena",
    price: "150.000",
    includes: [
      "Lancha rápida premium",
      "Almuerzo gourmet",
      "Visita Oceanario",
      "Playa Blanca",
      "Snorkel en arrecife",
      "Seguro de accidentes"
    ]
  },
  {
    id: 3,
    name: "Isla Barú Exclusivo",
    image: "https://images.pexels.com/photos/1174732/pexels-photo-1174732.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Playas cristalinas y tranquilidad absoluta",
    price: "100.000",
    includes: [
      "Transporte terrestre y marítimo",
      "Almuerzo caribeño",
      "Bebidas incluidas",
      "Acceso Playa Blanca",
      "Hamacas y sombrillas",
      "Tiempo libre 4 horas"
    ]
  },
  {
    id: 4,
    name: "Isla Grande Aventura",
    image: "https://images.pexels.com/photos/2156881/pexels-photo-2156881.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Explora la isla más grande del archipiélago",
    price: "130.000",
    includes: [
      "Tour en lancha",
      "Almuerzo típico",
      "Kayaking",
      "Paddle board",
      "Snorkel libre",
      "Guía especializado"
    ]
  },
  {
    id: 5,
    name: "Tour 5 Islas VIP",
    image: "https://images.pexels.com/photos/1450353/pexels-photo-1450353.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Recorre las 5 joyas del Caribe en un día",
    price: "180.000",
    includes: [
      "Lancha de lujo",
      "5 paradas estratégicas",
      "Almuerzo buffet",
      "Bar abierto",
      "Fotografía profesional",
      "Tour privado opcional"
    ]
  },
  {
    id: 6,
    name: "Sunset en Cholón",
    image: "https://images.pexels.com/photos/2049422/pexels-photo-2049422.jpeg?auto=compress&cs=tinysrgb&w=800",
    description: "Vive el atardecer más espectacular del Caribe",
    price: "140.000",
    includes: [
      "Salida tarde (3pm)",
      "Cena romántica",
      "Cócteles premium",
      "Música en vivo",
      "Atardecer en el mar",
      "Regreso nocturno"
    ]
  }
];

export const testimonials = [
  {
    id: 1,
    name: "María Fernanda Rodríguez",
    image: "https://randomuser.me/api/portraits/women/44.jpg",
    rating: 5,
    comment: "¡Experiencia inolvidable! El tour a Cholón superó todas mis expectativas. El servicio fue excelente y la comida deliciosa. Totalmente recomendado."
  },
  {
    id: 2,
    name: "Carlos Andrés Mejía",
    image: "https://randomuser.me/api/portraits/men/32.jpg",
    rating: 5,
    comment: "Las Islas del Rosario son un paraíso. La organización fue perfecta, los guías muy atentos y las actividades increíbles. Volveré sin duda."
  },
  {
    id: 3,
    name: "Laura Sofía Gómez",
    image: "https://randomuser.me/api/portraits/women/68.jpg",
    rating: 5,
    comment: "Mejor tour de mi vida. Playa Blanca es hermosa, el agua cristalina y el ambiente espectacular. Gracias por hacer de nuestro viaje algo especial."
  },
  {
    id: 4,
    name: "Juan Pablo Díaz",
    image: "https://randomuser.me/api/portraits/men/46.jpg",
    rating: 5,
    comment: "Excelente relación calidad-precio. Todo muy bien organizado, desde el punto de encuentro hasta el regreso. Las islas son un sueño hecho realidad."
  }
];
